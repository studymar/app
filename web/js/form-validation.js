 ...
 */
    ...
    VeeValidate.extend('ShortText', {
        validate: value => {
            return (/^[a-zA-ZäÄüÜöÖ0-9!§$%&/()=?ß\[\]+\-:\.;,@ <>]+$/.test(value)) && !(/script/i.test(value)) 
        },
        //validate: value => { return false },
        message: '{_field_} enthält unerlaubte Zeichen.'
    });

    VeeValidate.extend('LongText', {
        validate: value => {
            return (/^[a-zA-ZäÄüÜöÖ0-9!§$%&/()=?ß\[\]*+_\-:\.;,@ <>"]+$/.test(value)) && !(/script/i.test(value)) 
        },
        //validate: value => { return false },
        message: '{_field_} enthält unerlaubte Zeichen.'
    });
