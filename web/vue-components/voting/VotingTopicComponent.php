        ...
        <div v-if="topic.votingweights.length == 0 || (votingweight != null && topic.votingweights.length>0)">
            ...
            <div class="list-group" v-if="items != null && items.length>0" >
               <span v-for="(item,index) in items" :key="item.id">
                    <div class="list-group-item rounded mb-1" v-bind:class="[{'list-group-item-action': !item.hasAnswered},{'inactive': item.hasAnswered}]">
                        <a href="" class="stretched-link" v-on:click.prevent="$parent.openVote(item)" v-if="!item.hasAnswered"></a>
